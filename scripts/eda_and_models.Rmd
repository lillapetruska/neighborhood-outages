...
acs_outages_class <- 
  acs_outages %>% select(-c(GEOID, median_outage_duration_hr)) %>% drop_na()
```
...
acs_outages_class$pred <- 
  if_else(logit_mod$fitted.values > 0.5, "1", "0")

confusion_matrix <-
  table(acs_outages_class$above_median_cust_affected, acs_outages_class$pred)
...
efficiency <- sum(diag(confusion_matrix)) / sum(confusion_matrix)
efficiency
```

Accuracy of the model is about 80.6%.
