...
```{r}
# calculate mean 0-1 loss
base_01_loss <- 
  (confusion_matrix[1,2] + confusion_matrix[2,1]) / nrow(acs_outages_class)
base_01_loss
```

The mean 0-1 loss is about 0.19.

```{r, warning=FALSE, message=FALSE}
# Plot ROC Curve
roc(
  above_median_cust_affected ~ logit_mod$fitted.values, 
  data = acs_outages_class, 
  plot = TRUE,
  main = "Baseline ROC Curve",
  col= "blue"
)
```
...
Major goal is optimize mean 0-1 loss. Want high sensitivity, but also want to
generate as high specificity as possible (and reduce Type II error rate).
...
```
Around 16 components explains about 98% of variance.
...
pca_table <- 
  ...
  select(above_median_cust_affected:PC18) %>% 
  bind_cols()
```
...
knn_fit <-
  train(
    ...
    method = "knn",
    # Limit to k = sqrt(n)
    tuneGrid = expand.grid(k = 1:sqrt(nrow(acs_outages_class))),
...
class_validation <- 
  ...
  mutate(
    ...
    above_median_cust_affected =
      if_else(above_median_cust_affected == 1, "Yes", "No")
  ) %>% 
  slice_sample(prop = .2)
...
class_train <- 
  ...
  filter(!(rowid %in% class_validation$rowid)) %>% 
  select(-rowid)

class_validation <- class_validation %>% select(-rowid)

# Extract outcome variable for 'cl' argument
target_category <- class_train$above_median_cust_affected
# Extract outcome variable to measure accuracy
test_category <- class_validation$above_median_cust_affected
```
...
```{r}
# Plot results of fitted KNN model
plot(class_fit, print.thres = 0.5, type = "S")
```


```{r}
# Create confusion matrix
confusion_matrix_knn <- table(knn_mod, test_category)
rownames(confusion_matrix_knn) <- c("Obs. 0", "Obs. 1")
colnames(confusion_matrix_knn) <- c("Pred. 0", "Pred. 1")
...
model_tpr <- 
  confusion_matrix_knn[2,2] / 
  (confusion_matrix_knn[2,1] + confusion_matrix_knn[2,2])
model_tpr
```
...
# Plot ROC Curve
roc(
  above_median_cust_affected ~ as.numeric(knn_pred), 
  data = acs_outages_class, 
  plot = TRUE,
  main = "ROC Curve",
  col= "blue"
